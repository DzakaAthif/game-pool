 ...
 */
public class Ball extends Subject implements Drawable, Movable, Prototype {
    ...
    */
    ...
    public Ball clone() {
        ...
        newBall.setInitialXPos(this.originalPos[0]);
        newBall.setInitialYPos(this.originalPos[1]);
        newBall.setXPos(this.getXPos());
        newBall.setYPos(this.getYPos());
        newBall.setInitialXVel(this.originalVel[0]);
        newBall.setInitialYVel(this.originalVel[1]);
        newBall.setXVel(this.getXVel());
        newBall.setYVel(this.getYVel());
        newBall.setMass(this.getMass());
        newBall.setColour(this.colour.toString());
//        if (this.getBallType() == BallType.CUEBALL)
//            System.out.println("Cloning cueBall");
        newBall.setBallType(this.getBallType());
        newBall.setPocketAction(this.getPocketAction());

        if (this.isDisabled())
            newBall.disable();

        for (int i = 0; i < this.getFallCounter(); i++)
            newBall.incrementFallCounter();

        newBall.setPoints(this.getPoints());

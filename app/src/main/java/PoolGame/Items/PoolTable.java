...
//        this.pockets.add(new Pocket(dimX - POCKET_OFFSET, dimY - POCKET_OFFSET));
    ...
    public List<Pocket> getPockets() {return this.pockets;}

    public double getPocket_offset() {return this.pocket_offset;}
    /**
     * Set up table with the list of balls, which includes the CueBall.
     * @param balls The list of balls to be added to the table
     */
    public void setupBalls(List<Ball> balls, Score score) {
        this.score = score;
        ...
        for (Ball ball : balls) {
            ...
            // }
            ball.attach(new BallObserver(ball, score));
            this.addBall(ball);
        }
    }
    ...
    public void setupPockets(List<Pocket> pockets) {
        ...
        this.pocket_offset = pockets.get(0).getRadius()/2;
    }
    ...
    public Memento saveState() {
        ...
        for (Ball ball : balls) {
            ...
            newBall.attach(new BallObserver(newBall, score));

            newBalls.add(newBall);
        }

        return new Memento(newBalls, score.getPoints());
    }

    public void recoverState(Memento memento) {

        balls = memento.getBalls();
        score.setPoints(memento.getScore());
    }
}
